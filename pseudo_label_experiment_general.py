import numpy as np
from rkhs_glm_scaled import RKHSGLM_Scaled, sigmoid, softplus, KERNELS
...
def make_data(family, eta, rng, **params):
    ...
    if family == "gaussian":
        ...
        return eta, eta + rng.normal(0, sigma, size=n)
        ...
        return mu, rng.poisson(mu).astype(float)
        ...
        return mu, rng.poisson(lam).astype(float)
...
def mean(family, eta, **params):
    if family == "gaussian":
        ...
        return sigmoid(np.clip(eta, -10, 10))
...
class KGLM_covariate_shift:
    def __init__(self, n, n_0, B, fcn, family, kernel, seed, **kwargs): # preparations, sample generation
    ...
    def fit(self, rho = 0.5, beta = 2, lbd_min = None, lbd_max = None, lbd_tilde = None): # KRGLM under covariate shift
        ...
        mdl = RKHSGLM_Scaled(family=self.family, kernel=self.kernel, lam=lbd_tilde, fit_intercept=False)# clip_f=15, cg_tol=1e-7, precond="jacobi")
        ...
        for (j, lbd) in enumerate(self.Lambda):
            # KGLM
            mdl = RKHSGLM_Scaled(family=self.family, kernel=self.kernel, lam=lbd, fit_intercept=False)
